...
const nav: Item[] = [
  ...
  {
    to: "/reception/ipd-transactions",
    label: "Recent IPD Payments",
    icon: ListChecks,
  },
  {
    to: "/reception/diagnostic/token-generator",
    label: "Diagnostic Token Generator",
    icon: Ticket,
  },
  {
    to: "/reception/diagnostic/sample-tracking",
    label: "Diagnostic Sample Tracking",
    icon: ListChecks,
  },
  {
    to: "/reception/lab/sample-intake",
    label: "Lab Sample Intake",
    icon: Ticket,
  },
  {
    to: "/reception/lab/sample-tracking",
    label: "Lab Sample Tracking",
    icon: ListChecks,
  },
  {
    to: "/reception/lab/manager-cash-count",
    label: " Manager Cash Count",
    icon: Calculator,
  },
  {
    to: "/reception/sidebar-permissions",
...
const hiddenPaths = new Set([
  "/reception/diagnostic/token-generator",
  "/reception/diagnostic/sample-tracking",
]);

const baseNav = nav.filter((i) => !hiddenPaths.has(i.to));
...
export default function Reception_Sidebar({
  ...
  const [items, setItems] = useState<Item[]>(baseNav);

  useEffect(() => {
    try {
      const raw =
        localStorage.getItem("reception.user") ||
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res: any = await (receptionApi as any).listSidebarPermissions?.(
          role,
        );
        const doc = Array.isArray(res) ? res[0] : res;
        const map = new Map<string, any>();
        const perms = (doc?.permissions || []) as Array<{
          path: string;
          visible?: boolean;
          order?: number;
        }>;
        ...
        const computed = baseNav
          .filter((item) => {
            if (item.to === "/reception/sidebar-permissions" && !isAdmin)
              return false;
          ...
          .sort((a, b) => {
            ...
            const ia = baseNav.findIndex((n) => n.to === a.to);
            const ib = baseNav.findIndex((n) => n.to === b.to);
            return ia - ib;
          });
        if (mounted) setItems(computed);
      } catch {
        if (mounted) setItems(baseNav);
      }
    })();
    return () => {
      mounted = false;
  ...
  async function logout() {
    try {
      await receptionApi.logout();
    } catch {}
    try {
      localStorage.removeItem("reception.token");
      localStorage.removeItem("token");
      localStorage.removeItem("reception.user");
  ...
  return (
    ...
    >
      <nav
        className={`flex-1 overflow-y-auto ${collapsed ? "p-2" : "p-3"} space-y-1`}
      >
        {items.map((it) => {
          const Icon = it.icon;
          return (
      ...
      <div className={collapsed ? "p-2" : "p-3"}>
        <button
          ...
          className={
            collapsed
              ? "w-full inline-flex items-center justify-center rounded-md p-2 text-sm font-medium"
