...
import {
  getSavedPrescriptionPdfTemplate,
...
export default function Doctor_Prescription() {
  ...
  const [manualAttachmentError, setManualAttachmentError] =
    useState<string>("");
  ...
  const [activeTab, setActiveTab] = useState<
    "details" | "vitals" | "labs" | "diagnostics"
  >("details");
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res: any = await labApi.listTests({
          q: "",
          page: 1,
          limit: 2000,
        });
        ...
        const names = tests
          .map((t: any) => String(t?.name || t?.testName || t || "").trim())
          .filter(Boolean);
        if (!cancelled)
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const names = meds
          .map((m: any) => String(m?.name || m?.genericName || m || "").trim())
          .filter(Boolean);
        if (!cancelled)
          setMedNameSuggestions(Array.from(new Set(names)).slice(0, 2000));
      } catch {
        // ignore
      }
    })();
    ...
    (async () => {
      try {
        ...
        for (const r of rows) {
          ...
          if (r.primaryComplaintHistory)
            primHist.push(String(r.primaryComplaintHistory));
          ...
          if (Array.isArray(r.labTests))
            labTestsAll.push(
              ...(r.labTests as any[]).map((x) => String(x || "")),
            );
          if (r.labNotes) labNotes.push(String(r.labNotes));
          if (Array.isArray(r.diagnosticTests))
            diagTestsAll.push(
              ...(r.diagnosticTests as any[]).map((x: any) => String(x || "")),
            );
          ...
          try {
            ...
            if (vv.bloodPressureSys != null)
              vSys.push(String(vv.bloodPressureSys));
            if (vv.bloodPressureDia != null)
              vDia.push(String(vv.bloodPressureDia));
            if (vv.respiratoryRate != null)
              vResp.push(String(vv.respiratoryRate));
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const depArray: any[] = ((depRes as any)?.departments ||
          (depRes as any) ||
          []) as any[];
        ...
        const deptName = d?.primaryDepartmentId
          ? depArray.find(
              (z: any) =>
                String(z._id || z.id) === String(d.primaryDepartmentId),
            )?.name || ""
          : "";
        if (d)
          setDoctorInfo({
  ...
  async function loadPresIds() {
    try {
      ...
      const ids: string[] = (res.prescriptions || []).map((p: any) =>
        String(p.encounterId?._id || p.encounterId || ""),
      );
  ...
  useEffect(() => {
    ...
    if (!tokenId) return;
    const t = myPatients.find(
      (x) =>
        String(x.id) === String(tokenId) ||
        String((x as any)._id) === String(tokenId),
    );
    if (!t) return;
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // meds mapping
        const meds: MedicineRow[] = (pres.items || [])
          .map((it: any) => {
            const notes = String(it?.notes || "").trim();
            let instruction = "";
            let route = "";
            try {
              const mi = notes.match(/Instruction:\s*([^;]+)/i);
              if (mi && mi[1]) instruction = mi[1].trim();
            } catch {}
            try {
              const mr = notes.match(/Route:\s*([^;]+)/i);
              if (mr && mr[1]) route = mr[1].trim();
            } catch {}
            return {
        ...
        const labTestsText = Array.isArray(pres.labTests)
          ? pres.labTests
              .map((x: any) => String(x || "").trim())
              .filter(Boolean)
              .join("\n")
          : "";
        const diagTestsText = Array.isArray(pres.diagnosticTests)
          ? pres.diagnosticTests
              .map((x: any) => String(x || "").trim())
              .filter(Boolean)
              .join("\n")
          : "";
        ...
        setForm((f) => ({
          ...
          primaryComplaint: String(
            pres.primaryComplaint || pres.complaints || "",
          ),
          ...
          diagDisplay: {
            testsText: diagTestsText,
            notes: String(pres.diagnosticNotes || ""),
          },
        ...
        try {
          vitalsRef.current?.setDisplay?.(pres.vitals || {});
        } catch {}
        try {
          diagRef.current?.setDisplay?.({
            testsText: diagTestsText,
            notes: String(pres.diagnosticNotes || ""),
          });
        } catch {}
  ...
  async function searchMedicines(q: string) {
    try {
      ...
      const suggestions: any[] =
        res?.suggestions ?? res?.medicines ?? res?.items ?? res ?? [];
  ...
  const save = async (e: React.FormEvent) => {
        ...
        frequency:
          m.freqText && m.freqText.trim()
            ...
            : ["morning", "noon", "evening", "night"]
                ...
                .join("/ ") || undefined,
        duration:
          m.durationText && m.durationText.trim()
            ? m.durationText.trim()
            : m.days
              ? `${m.days} ${m.durationUnit || "day(s)"}`
              : undefined,
        notes:
          m.route || m.instruction
            ? [
    ...
    if (rxMode === "manual") {
      if (manualAttachmentError) {
        alert(manualAttachmentError);
        return;
      }
      if (!manualAttachment?.dataUrl) {
        alert("Attach prescription PDF/image for Manual mode");
        return;
      }
    } else {
      if (!items.length) {
    ...
    const labTests = form.labTestsText
      .split(/\n|,/)
      .map((s) => s.trim())
      .filter(Boolean);
    try {
      ...
      try {
        vRaw = vitalsRef.current?.getNormalized?.();
      } catch {}
      const hasVitals =
        vRaw &&
        Object.values(vRaw).some(
          (x) => x != null && !(typeof x === "number" && isNaN(x)),
        );
      let vitals: any = undefined;
      if (hasVitals) vitals = vRaw;
      else if (Object.keys((form as any).vitalsNormalized || {}).length)
        vitals = (form as any).vitalsNormalized;
      else if (
        (form as any).vitalsDisplay &&
        Object.values((form as any).vitalsDisplay).some(Boolean)
      ) {
      ...
      const diagnosticTests =
        Array.isArray(dRaw?.tests) && dRaw?.tests?.length
          ? dRaw?.tests
          : undefined;
      ...
      await hospitalApi.createPrescription({
        ...
        manualAttachment:
          rxMode === "manual" ? manualAttachment || undefined : undefined,
      ...
      try {
        diagRef.current?.setDisplay?.({ testsText: "", notes: "" });
      } catch {}
  ...
  async function openPrint() {
    ...
    let s: any = settings;
    try {
      s = (await hospitalApi.getSettings()) as any;
    } catch {}
    const settingsNorm = {
    ...
    try {
      if (sel?.mrNo) {
        ...
        if (p) {
          ...
          patient = {
            name: p.fullName || sel.patientName || "-",
            mrn: p.mrn || sel.mrNo || "-",
            gender: p.gender || "-",
    ...
    try {
      const d = vitalsRef.current?.getDisplay?.();
      if (d) setForm((f) => ({ ...f, vitalsDisplay: d }));
    } catch {}
    let vRaw = undefined as any;
    try {
      vRaw = vitalsRef.current?.getNormalized?.();
    ...
    await previewPrescriptionPdf(
      {
        ...
        advice: form.advice,
        vitals,
        labTests: form.labTestsText
          .split(/\n|,/)
          .map((s) => s.trim())
          .filter(Boolean),
        labNotes: form.labNotes,
        diagnosticTests: Array.isArray(dPrint.tests) ? dPrint.tests : [],
        diagnosticNotes: dPrint.notes || "",
        createdAt: new Date(),
      },
      tpl,
    );
  ...
  const goTab = (tab: "details" | "vitals" | "labs" | "diagnostics") => {
    if (activeTab === "vitals") {
      try {
        ...
        setForm((f) => ({
          ...f,
          vitalsDisplay: disp || f.vitalsDisplay,
          vitalsNormalized: norm || f.vitalsNormalized,
        }));
      } catch {}
    }
  ...
  async function referToDiagnosticQuick() {
    ...
    try {
      ...
      const tests =
        Array.isArray(d.tests) && d.tests.length ? d.tests : undefined;
      const notes =
        d.notes && String(d.notes).trim() ? String(d.notes).trim() : undefined;
      await hospitalApi.createReferral({
        type: "diagnostic",
        encounterId: sel.encounterId,
        doctorId: doc.id,
        tests,
  ...
  const keyFor = (name: string) =>
    `doctor.suggest.${doc?.id || "anon"}.${name}`;
  ...
  const saveList = (name: string, values: string[]) => {
    ...
    try {
      localStorage.setItem(keyFor(name), JSON.stringify(uniq.slice(0, 200)));
    } catch {}
  };
  ...
  const addMany = (name: string, values: string[]) => {
    const arr = loadList(name);
    const next = [
      ...values.map((s) => String(s || "").trim()).filter(Boolean),
      ...arr,
    ];
  ...
  const sugPrimary = useMemo(
    () => loadList("primaryComplaint"),
    [doc?.id, sugVersion],
  );
  const sugHistory = useMemo(() => loadList("history"), [doc?.id, sugVersion]);
  const sugPrimHist = useMemo(
    () => loadList("primaryComplaintHistory"),
    [doc?.id, sugVersion],
  );
  const sugFamily = useMemo(
    () => loadList("familyHistory"),
    [doc?.id, sugVersion],
  );
  const sugAllergy = useMemo(
    () => loadList("allergyHistory"),
    [doc?.id, sugVersion],
  );
  const sugTreatment = useMemo(
    () => loadList("treatmentHistory"),
    [doc?.id, sugVersion],
  );
  const sugExam = useMemo(
    () => loadList("examFindings"),
    [doc?.id, sugVersion],
  );
  const sugDiagnosis = useMemo(
    () => loadList("diagnosis"),
    [doc?.id, sugVersion],
  );
  const sugAdvice = useMemo(() => loadList("advice"), [doc?.id, sugVersion]);
  const sugLabTests = useMemo(
    () =>
      Array.from(
        new Set([
          ...(labTestSuggestions.length ? labTestSuggestions : labSeeds),
          ...loadList("labTest"),
        ]),
      ),
    [doc?.id, sugVersion, labTestSuggestions],
  );
  const sugLabNotes = useMemo(
    () => loadList("labNotes"),
    [doc?.id, sugVersion],
  );
  const sugDiagTests = useMemo(
    () => loadList("diagTest"),
    [doc?.id, sugVersion],
  );
  const sugDiagNotes = useMemo(
    () => loadList("diagNotes"),
    [doc?.id, sugVersion],
  );
  const sugDose = useMemo(
    () =>
      Array.from(new Set([...(seedsDose as string[]), ...loadList("dose")])),
    [doc?.id, sugVersion],
  );
  const sugInstr = useMemo(
    () =>
      Array.from(
        new Set([
          ...(seedsInstruction as string[]),
          ...loadList("instruction"),
        ]),
      ),
    [doc?.id, sugVersion],
  );
  const sugRoute = useMemo(
    () =>
      Array.from(new Set([...(seedsRoute as string[]), ...loadList("route")])),
    [doc?.id, sugVersion],
  );
  const sugDuration = useMemo(
    () =>
      Array.from(
        new Set([...(seedsDuration as string[]), ...loadList("durationTag")]),
      ),
    [doc?.id, sugVersion],
  );
  const sugFreq = useMemo(
    () =>
      Array.from(
        new Set([...(seedsFrequency as string[]), ...loadList("frequencyTag")]),
      ),
    [doc?.id, sugVersion],
  );
  // Vitals suggestions
  const sugVPulse = useMemo(
    () => loadList("vitals.pulse"),
    [doc?.id, sugVersion],
  );
  const sugVTemp = useMemo(
    () => loadList("vitals.temperature"),
    [doc?.id, sugVersion],
  );
  const sugVSys = useMemo(() => loadList("vitals.sys"), [doc?.id, sugVersion]);
  const sugVDia = useMemo(() => loadList("vitals.dia"), [doc?.id, sugVersion]);
  const sugVResp = useMemo(
    () => loadList("vitals.resp"),
    [doc?.id, sugVersion],
  );
  const sugVSugar = useMemo(
    () => loadList("vitals.sugar"),
    [doc?.id, sugVersion],
  );
  const sugVWeight = useMemo(
    () => loadList("vitals.weight"),
    [doc?.id, sugVersion],
  );
  const sugVHeight = useMemo(
    () => loadList("vitals.height"),
    [doc?.id, sugVersion],
  );
  const sugVSpo2 = useMemo(
    () => loadList("vitals.spo2"),
    [doc?.id, sugVersion],
  );
  ...
  return (
    <div className="w-full px-2 sm:px-4">
        ...
        >
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setForm((f) => ({ ...f, patientKey: e.target.value }))
              }
          ...
          {rxMode === "manual" && (
            <div>
              <label className="mb-1 block text-sm text-slate-700">
                Attach Manual Prescription (PDF/Image)
              </label>
              <input
                type="file"
              ...
              {manualAttachment?.dataUrl &&
                (manualAttachment.mimeType || "").startsWith("image/") && (
                  <div className="mt-2">
                    <img
                      src={manualAttachment.dataUrl}
          ...
          <div className="mt-2 border-b border-slate-200">
            <nav className="-mb-px flex gap-2">
              <button
                ...
                className={`px-3 py-2 text-sm ${activeTab === "details" ? "border-b-2 border-sky-600 text-slate-900" : "text-slate-600 hover:text-slate-900"}`}
              >
                Details
              </button>
              <button
                type="button"
                onClick={() => goTab("vitals")}
                className={`px-3 py-2 text-sm ${activeTab === "vitals" ? "border-b-2 border-sky-600 text-slate-900" : "text-slate-600 hover:text-slate-900"}`}
              >
                Vitals
              </button>
              <button
                type="button"
                onClick={() => goTab("labs")}
                className={`px-3 py-2 text-sm ${activeTab === "labs" ? "border-b-2 border-sky-600 text-slate-900" : "text-slate-600 hover:text-slate-900"}`}
              >
                Lab Orders
              </button>
              <button
                type="button"
                onClick={() => goTab("diagnostics")}
                className={`px-3 py-2 text-sm ${activeTab === "diagnostics" ? "border-b-2 border-sky-600 text-slate-900" : "text-slate-600 hover:text-slate-900"}`}
              >
                Diagnostic Orders
              </button>
            </nav>
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <div>
                  ...
                  <SuggestField
                    ...
                    onChange={(v) =>
                      setForm((f) => ({ ...f, primaryComplaint: v }))
                    }
                ...
                <div>
                  ...
                  <SuggestField
                    ...
                    onChange={(v) =>
                      setForm((f) => ({ ...f, primaryComplaintHistory: v }))
                    }
                ...
                <div>
                  ...
                  <SuggestField
                    ...
                    onChange={(v) =>
                      setForm((f) => ({ ...f, familyHistory: v }))
                    }
                    suggestions={sugFamily}
                  />
                </div>
                <div>
                  <label className="mb-1 block text-sm text-slate-700">
                  ...
                  <SuggestField
                    ...
                    onChange={(v) =>
                      setForm((f) => ({ ...f, allergyHistory: v }))
                    }
                    suggestions={sugAllergy}
                  />
                </div>
                <div>
                  <label className="mb-1 block text-sm text-slate-700">
                  ...
                  <SuggestField
                    ...
                    onChange={(v) =>
                      setForm((f) => ({ ...f, treatmentHistory: v }))
                    }
                    suggestions={sugTreatment}
                  />
                </div>
                <div>
                  <label className="mb-1 block text-sm text-slate-700">
          ...
          {activeTab === "vitals" && (
            <div>
              <PrescriptionVitals
                ...
                onBlurStore={(field: any, value: string) =>
                  addOne(`vitals.${String(field)}`, value)
                }
              />
            </div>
          )}
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div>
                ...
                <SuggestField
                  rows={2}
                  value={form.labNotes}
                  onChange={(v) => setForm((f) => ({ ...f, labNotes: v }))}
                  suggestions={sugLabNotes}
                />
              </div>
            </div>
          )}
          <div className="flex items-center justify-end gap-2">
            ...
            <button
              ...
              className="btn-outline-navy disabled:opacity-50"
            >
              Refer to IPD
            </button>
      ...
      {openReferral && (
        <div
          id="referral-print"
          className="fixed inset-0 z-50 grid place-items-center bg-black/40 px-2 sm:px-4"
        >
          ...
          <div className="w-full max-w-4xl rounded-xl bg-white shadow-2xl ring-1 ring-black/5">
            <div className="flex items-center justify-between border-b border-slate-200 px-4 py-3">
              ...
              <div className="flex items-center gap-2">
                <button onClick={printReferral} className="btn-outline-navy">
                  Print
                </button>
                <button onClick={() => setOpenReferral(false)} className="btn">
                  Close
                </button>
              </div>
            </div>
