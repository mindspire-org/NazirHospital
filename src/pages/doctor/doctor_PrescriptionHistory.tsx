...
export default function Doctor_PrescriptionHistory() {
  ...
  const [confirmDlg, setConfirmDlg] = useState<{
    open: boolean;
    type?: "lab" | "pharmacy" | "diagnostic" | "delete";
  ...
  const [refFlags, setRefFlags] = useState<
    Record<string, { ph?: boolean; lab?: boolean; diag?: boolean }>
  >({});
  ...
  async function load() {
    try {
      ...
      try {
        ...
        const map: Record<
          string,
          { ph?: boolean; lab?: boolean; diag?: boolean }
        > = {};
        (ph?.referrals || []).forEach((r: any) => {
          const pid = String(r.prescriptionId || "");
          if (!pid) return;
          map[pid] = { ...(map[pid] || {}), ph: true };
        });
        (lb?.referrals || []).forEach((r: any) => {
          const pid = String(r.prescriptionId || "");
          if (!pid) return;
          map[pid] = { ...(map[pid] || {}), lab: true };
        });
        (dg?.referrals || []).forEach((r: any) => {
          const pid = String(r.prescriptionId || "");
          if (!pid) return;
          map[pid] = { ...(map[pid] || {}), diag: true };
        });
        setRefFlags(map);
      } catch {}
    } catch {
  ...
  async function openAttachment(p: Prescription) {
    try {
      if (p.manualAttachment?.dataUrl) {
        setAttachView({
          open: true,
          fileName: p.manualAttachment.fileName,
          mimeType: p.manualAttachment.mimeType,
          dataUrl: p.manualAttachment.dataUrl,
        });
        return;
      }
  ...
  async function referToPharmacy(p: Prescription) {
    try {
      if (!doc?.id) {
        setToast({ msg: "Session missing", kind: "error" });
        return;
      }
      ...
      setRefFlags((prev) => ({
        ...prev,
        [p.id]: { ...(prev[p.id] || {}), ph: true },
      }));
    } catch (e: any) {
      setToast({
        msg: e?.message || "Failed to refer to pharmacy",
        kind: "error",
      });
    }
  }
  ...
  async function referToLab(p: Prescription) {
    try {
      ...
      setRefFlags((prev) => ({
        ...prev,
        [p.id]: { ...(prev[p.id] || {}), lab: true },
      }));
    } catch (e: any) {
      setToast({
        msg: e?.message || "Failed to create lab referral",
        kind: "error",
      });
    }
  }
  ...
  async function referToDiagnostic(p: Prescription) {
    try {
      ...
      try {
        const res: any = await hospitalApi.getPrescription(p.id);
        const pres = res?.prescription;
        if (pres?.diagnosticTests?.length)
          tests = pres.diagnosticTests as string[];
      ...
      setRefFlags((prev) => ({
        ...prev,
        [p.id]: { ...(prev[p.id] || {}), diag: true },
      }));
    } catch (e: any) {
      setToast({
        msg: e?.message || "Failed to create diagnostic referral",
        kind: "error",
      });
    }
  }
  ...
  function openConfirm(
    ...
    if (type === "pharmacy" && refFlags[p.id]?.ph) {
      setToast({ msg: "Already referred to pharmacy", kind: "success" });
      return;
    }
    if (type === "lab" && refFlags[p.id]?.lab) {
      setToast({ msg: "Already referred to lab", kind: "success" });
      return;
    }
    if (type === "diagnostic" && refFlags[p.id]?.diag) {
      setToast({ msg: "Already referred to diagnostic", kind: "success" });
      return;
    }
  ...
  async function confirmReferral() {
    ...
    if (!confirmDlg.open || !confirmDlg.type || !p) {
    ...
    <div className="space-y-4">
              ...
              <div key={p.id} className="px-4 py-3 text-sm">
                ...
                <div className="mt-2 flex flex-wrap items-center gap-2">
                  ...
                  {p.prescriptionMode === "manual" &&
                    (p.manualAttachment?.dataUrl ||
                      p.manualAttachment?.fileName) && (
                      <button
                        className="rounded-md border border-slate-300 px-3 py-1 text-sm"
                  ...
                  {(() => {
                    const done = !!refFlags[p.id]?.ph;
                    return (
                      <button
                        className={`rounded-md border px-3 py-1 text-sm ${done ? "border-slate-200 bg-slate-100 text-slate-500" : "border-slate-300 hover:bg-slate-50"}`}
                        disabled={done}
          ...
          <div className="w-full max-w-sm rounded-xl bg-white p-4">
            ...
            <div className="text-sm text-slate-700">
              {confirmDlg.type === "delete"
                ...
                : `Create a ${confirmDlg.type === "pharmacy" ? "Pharmacy" : confirmDlg.type === "lab" ? "Lab" : "Diagnostic"} referral for this prescription?`}
            </div>
            <div className="mt-4 flex items-center justify-end gap-2">
              ...
              <button onClick={confirmReferral} className="btn">
                Confirm
              </button>
            </div>
          </div>
      ...
      {attachView.open && (
        <div
          ...
          onClick={() => setAttachView({ open: false })}
        >
          <div
            className="w-full max-w-4xl rounded-xl bg-white p-4"
          ...
          >
            <div className="flex items-center justify-between gap-2">
              ...
              <div className="flex items-center gap-2">
                {attachView.dataUrl && (
                  <a
                    className="rounded-md border border-slate-300 px-3 py-1 text-sm"
                    href={attachView.dataUrl}
                    download={attachView.fileName || "prescription-attachment"}
                  >
                    Download
                  </a>
                )}
            ...
            <div className="mt-3 max-h-[75vh] overflow-auto rounded-lg border border-slate-200 p-2">
              {attachView.dataUrl &&
                (attachView.mimeType || "").startsWith("image/") && (
                  <img
                    src={attachView.dataUrl}
                    alt="Attachment"
                    className="mx-auto max-h-[70vh]"
                  />
                )}
              {attachView.dataUrl &&
                (attachView.mimeType || "").includes("pdf") && (
                  <iframe
                    title="Attachment"
                    src={attachView.dataUrl}
